import {onSnapshot, doc,query, orderBy, collection, getDoc, addDoc, updateDoc, getDocs, where, writeBatch} from "firebase/firestore";
...
//     await batch.commit();
    ...
    ventaData.lista.forEach(async (item)=>{
        ...
        const q = query(collection(db, `Tienda/${idTienda}/Producto`), where("idProducto", "==", item.idProducto));
        ...
        await updateDoc(productRef, {cantidad: docProducto.data().cantidad-item.cantidad});
    })
